...
const MOCK = {
  ...
  phases: [
    {
      title: "Phase 1｜穩定基本盤（1–2個月）", goal: "提高新觀眾滲透率＋留存",
      bullets: ["固定主力內容（避免頻繁換主題）", "每週至少 2 支精華剪輯（高密度笑點/衝突點）", "直播前 60 秒開場固定模板：主題 → 今天目標 → 鉤子事件"],
      kpi: ["留存 +5~10%", "外部導流 +20%", "ACV 穩定上行"],
    },
    {
      title: "Phase 2｜製造爆點（3–6個月）", goal: "建立談資與話題事件",
      bullets: ["月度企劃：連續挑戰 / 觀眾投票路線 / 連動串台", "設計『可剪輯』橋段：關卡賭注、懲罰、里程碑儀式感", "合作名單分級：同量級 70%＋上位曝光 30%"],
      kpi: ["爆款剪輯 ≥2/月", "新觀眾占比 +15%", "合作導流 +30%"],
    },
    {
      title: "Phase 3｜品牌化（6–12個月）", goal: "IP 固化與可複製變現",
      bullets: ["建立專屬口頭禪/儀式、視覺標識、固定音效", "開啟商品化測試：貼圖、週邊、小額會員福利", "社群文化規則：新手歡迎、內梗字典、回流機制"],
...
function MiniStat({ icon: Icon, label, value, sub, warning, pulseHighlight, trend }) {
  return (
    <Card className={`p-4 flex flex-col justify-center transition-all hover:shadow-md ${pulseHighlight ? 'ring-1 ring-red-400/50' : ''}`}>
      <div className="flex items-start gap-3">
        <div className={`rounded-xl border border-neutral-200/70 p-2 shrink-0 ${pulseHighlight ? 'bg-red-50' : 'bg-neutral-50'}`}>
        ...
        <div className="flex flex-col min-w-0">
          ...
          <div className="mt-1 flex items-baseline gap-1 flex-wrap">
            ...
            {sub && <span className="text-[10px] text-neutral-400 font-medium uppercase">{sub}</span>}
          </div>

          {trend && (
            <div className={`mt-1.5 flex items-center gap-0.5 text-[11px] font-bold ${trend.trend >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
              {trend.trend >= 0 ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
              {trend.label}
              <span className="text-neutral-400 font-normal ml-1 text-[10px]">vs 前30日</span>
            </div>
          )}
...
export default function WarRoomNayabnb() {
  ...
  const statsComparison = useMemo(() => {
    const currentACV = twitchData?.isLive ? twitchData.currentViewers : MOCK.kpis.avgConcurrent.value;
    const acvGrowth = getGrowth(currentACV, PREV_30D_MOCK.acv);
    
    // 若沒有 Twitch API 真實追隨者，先用假資料算一個增長，有真實資料就用真實計算
    const currentFollowers = twitchData?.totalFollowers || 13500; 
    const followersGrowth = getGrowth(currentFollowers, PREV_30D_MOCK.followers);

    const retGrowth = getGrowth(MOCK.kpis.retentionProxy.value, PREV_30D_MOCK.retention);
    const engGrowth = getGrowth(MOCK.kpis.chatEngagement.value, PREV_30D_MOCK.engagement);
    const monGrowth = getGrowth(MOCK.kpis.monetization.value, PREV_30D_MOCK.monetization);
    ...
    return {
      acv: formatTrend(acvGrowth),
      followers: formatTrend(followersGrowth),
      retention: formatTrend(retGrowth),
      engagement: formatTrend(engGrowth),
      monetization: formatTrend(monGrowth),
      subGrowth: formatTrend(getGrowth(MOCK.kpis.subGrowthRate.value, PREV_30D_MOCK.subGrowthRate))
    };
  }, [twitchData]);
    ...
    <div className="min-h-screen bg-neutral-50 p-4 md:p-8 font-sans">
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
          <MiniStat 
            icon={TrendingUp} label="總追隨人數" 
            value={twitchData?.totalFollowers != null ? formatK(twitchData.totalFollowers) : "..."} 
            sub="Followers" trend={statsComparison.followers} 
          />
          <MiniStat 
            icon={Users} label={viewersLabel} value={formatK(displayViewers)} sub={viewersSub} 
            warning={viewersWarning} pulseHighlight={twitchData?.isLive} trend={statsComparison.acv} 
          />
          <MiniStat icon={Activity} label={MOCK.kpis.retentionProxy.label} value={`${Math.round(MOCK.kpis.retentionProxy.value * 100)}%`} sub={MOCK.kpis.retentionProxy.unit} trend={statsComparison.retention} />
          <MiniStat icon={Flame} label={MOCK.kpis.chatEngagement.label} value={`${Math.round(MOCK.kpis.chatEngagement.value * 100)}%`} sub={MOCK.kpis.chatEngagement.unit} trend={statsComparison.engagement} />
          <MiniStat icon={Wallet} label={MOCK.kpis.monetization.label} value={`${Math.round(MOCK.kpis.monetization.value * 100)}%`} sub={MOCK.kpis.monetization.unit} warning={MOCK.kpis.monetization.warning} trend={statsComparison.monetization} />
          <MiniStat icon={Crown} label={MOCK.kpis.subGrowthRate.label} value={`+${Math.round(MOCK.kpis.subGrowthRate.value * 100)}%`} sub={MOCK.kpis.subGrowthRate.unit} trend={statsComparison.subGrowth} />
        </motion.div>
        ...
        <CardBody className="h-[320px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={trendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <RechartsTooltip contentStyle={{ borderRadius: '12px', border: '1px solid #e5e5e5', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
    ...
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
      <Card>
        ...
        <CardBody className="h-[320px]">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <RechartsTooltip contentStyle={{ borderRadius: '12px', border: '1px solid #e5e5e5' }} />
